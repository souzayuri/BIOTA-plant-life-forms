...
path <- "C:/Users/Layu/Downloads/01_dados/contributions/"
setwd(path)


# Load data table through windows ----------------------------------------------------------
...
# Load data table through Mac ----------------------------------------------------------

#data_biota <- read_csv("biota_valesca_2019_ferns_v2.csv") # If all the columns is imported as agregated, try to load with read_csv2 function 
#glimpse(data_biota)
...
data.biota.phyl <- data_biota %>%
  ...
  rename(Site = site, Treatment = treatment, Plot = plot, Abundances = abundances, Month = month, Time = time)
...
data_biota.gather <- data_biota %>%
  dplyr::select(-c(1:5,23:26,29)) %>% # remove unused columns
  gather(key = "Month", value = "value", 1:17) %>% 
  dplyr::select(-c(3:4))
data_biota.gather


table1 <- data.biota.phyl %>% 
  right_join(data_biota.gather, by = "Species")

# extracting to a csv file ---------------------------------------------------

write_csv(data.biota.phyl, "biota_phylogenetic_datas.csv")
