...
data_biota_diver <- read_csv("C:/Users/Yuri/Google Drive/Yuri/Mestrado/parcelas_biota/projeto/PPG/tese/analises/01_dados/diversity/inv_sim_div_20-04-20.csv") # If all the columns is imported as agregated, try to load with read_csv2 function 
glimpse(data_biota_diver)

data.biota.div.mean <- data_biota_diver %>% 
  select(-c(1,2,4:5, 13:30)) %>% 
  gather(key = "Life_form", value = "inv.simp.div", 3:8) %>% 
  gather(key = "Life_form_2", value = "abundance", 3:8) %>% 
  select(-c(5)) %>% 
  dplyr::group_by(Time, Treatment, Life_form) %>% 
  summarise(mean_inv_sim = mean(inv.simp.div),
...
lf_div_open <- ggplot(subset(data.biota.div.mean, treatment %in% ("open")), 
                      aes(x= total_abundance, y= mean_inv_sim, size = time, 
  ...
  scale_colour_manual(values = c("springgreen4","yellowgreen","khaki4",
                                 "tan1","sienna3","brown")) +
  labs(y = "Inverse Simpson index", x = "Abundance", title = "",
...
ggsave("C:/Users/Yuri/Google Drive/Yuri/Mestrado/parcelas_biota/projeto/PPG/tese/analises/02_figuras/diversity/inv_simpsons_index_diversity_lf_open.jpeg", width = 15, height = 17, units = "cm", dpi = 1000)
...
mon.geomalpha.div.open <- ggplot(subset(data.biota.div.mean, treatment %in% ("open")), 
  ...
  scale_colour_manual(values = c("springgreen4","yellowgreen","khaki4",
                                 "tan1","sienna3","brown")) +
  labs(y = "Inverse Simpson index"~alpha, x = "Abundance", title = "",
       size = "Year", alpha = "Year",colour="Life-forms") +
  guides(colour = guide_legend(override.aes = list(size=3, stroke=3),order = 2),
         size = guide_legend(order = 1),
         alpha = guide_legend(order = 1)) +
...
ggsave("C:/Users/Yuri/Google Drive/Yuri/Mestrado/parcelas_biota/projeto/PPG/tese/analises/02_figuras/diversity/inv_simpson_index_diversity_lf_join.jpeg", width = 25, height = 25, units = "cm", dpi = 300)
